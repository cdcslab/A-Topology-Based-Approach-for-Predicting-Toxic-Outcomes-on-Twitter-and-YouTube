...
football_original_posts <- football_original_posts %>% 
...
load(posts_filename)
football_comments <- df 

football_comments <- football_comments %>% 
  filter(is.na(in_reply_to_id) == F) %>% 
  mutate(created_at = as_datetime(created_at)) %>%
  distinct(id, .keep_all = T)

conversation_ids_for_search = football_original_posts %>%
  group_by(conversation_id) %>%
  dplyr::summarise(n = n()) %>%
  ungroup()

conversation_ids_for_search <- conversation_ids_for_search %>% 
  filter(conversation_id %in% unique(football_comments$conversation_id))
...
existing_conversation_ids$conversation_ids = sapply(existing_conversation_ids$filename,
                                                    function(x) str_split(x,
                                                                          "shuffled_generative_graph_features_for_conversation-id_*")[[1]][2],
                                                    simplify = T)
...
# unique_conversation_ids = unique_conversation_ids[unique_conversation_ids %in% football_comments$conversation_id]
...
for (c_id in unique_conversation_ids[i: length(unique_conversation_ids)])
{
  ...
  output_filename = paste(
    output_folder,
    "shuffled_generative_graph_features_for_conversation-id_",
    c_id,
    ".csv",
    sep = ""
  )
  ...
  original_post_info = football_original_posts %>%
    filter(id == c_id) %>%
  ...
  football_comments_by_conversation_id <- football_comments %>%
  ...
  football_comments_by_conversation_id$in_reply_to_id <- sample(football_comments_by_conversation_id$in_reply_to_id )
  
  # The list of edges is initially populated with the children of the
  # root (the post)
  df_children = football_comments_by_conversation_id %>%
  ...
  while (number_of_children_left > 0)
  {
    ...
    # Add new children
    new_children = football_comments_by_conversation_id %>%
