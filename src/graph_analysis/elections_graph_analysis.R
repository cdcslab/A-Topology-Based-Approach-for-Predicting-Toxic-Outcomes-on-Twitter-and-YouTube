...
topic = "elections"
load_comments = TRUE
is_toxicity_shuffling_enabled = FALSE
threshold = 0.6
...
#                                         "retweeted_id" = "character",
#                                         "quoted_id" = "character",
#                                         "replied_id" = "character",
...
{
  load(
    "/media/gabett/Volume/data-repository/panconesi-football-elections/elections/comments/labelled/elections_comment_labelled_unified.RData"
  )
  ...
  save(df_comments,
       file = "/media/gabett/Volume/data-repository/panconesi-football-elections/elections/comments/labelled/elections_comments_unified_labelled_preprocessed.RData",
       version = 2)
} else {
  load(
    "/media/gabett/Volume/data-repository/panconesi-football-elections/elections/comments/labelled/elections_comments_unified_labelled_preprocessed.RData"
  )
}
...
for (c_id in unique_conversation_ids)
{
  ...
  df_comments_by_conversation_id$in_reply_to_id = as.character(df_comments_by_conversation_id$in_reply_to_id)
  ...
  cat("i:", i, "root:", c_id, "\n")
  i = i + 1
  edge_list = create_tree_edge_list(comment_df = df_comments_by_conversation_id)
  ...
  else
  {
    ...
    {
      tree_filename = paste(
        "/media/gabett/Volume/data-repository/panconesi-football-elections/elections/trees/igraph_data/shuffled/",
        "shuffled_tree_",
        c_id,
        ".csv",
        sep = ""
    ...
    # Toxicity ratio
    toxicity_ratio = get_toxicity_ratio(df_toxic_comments_by_conversation_id,
                                        df_comments_by_conversation_id,
                                        c_id)
    ...
    # Assortativity
    tryCatch({
      assortativity = get_assortativity(tree_lcc, c_id, "is_toxic")
    },
    error = function(e) {
      assortativity = NaN
    })
