...
folder <- "/media/gabett/Volume/data-repository/panconesi-football-elections/elections/youtube/"
posts_filename = paste(folder, "youtube_elections_original_videos_entire_dataset_scored.parquet", sep = "")
comments_filename = paste(folder, "youtube_elections_entire_dataset.parquet", sep = "")
output_folder = "/media/gabett/Volume/data-repository/panconesi-football-elections/elections/youtube/trees/shuffled_graph_analysis_for_each_comment/"
...
existing_video_ids$video_ids = sapply(existing_video_ids$filename,
                                                    function(x) str_split(x,
                                                                          "shuffled_generative_graph_features_for_video-id_*")[[1]][2],
                                                    simplify = T)
...
from_id = 1 #which("JfsPX7l2UnA" == unique_video_ids)
...
for (v_id in unique_video_ids[from_id:length(unique_video_ids)])
{
  ...
  output_filename = paste(
    output_folder,
    "shuffled_generative_graph_features_for_video-id_",
    v_id,
    ".csv",
    sep = ""
  )
  ...
  # After having prepared the graph, we shuffle the nodes
  df_comments_by_video_id$in_reply_to_id <- sample(df_comments_by_video_id$in_reply_to_id)
