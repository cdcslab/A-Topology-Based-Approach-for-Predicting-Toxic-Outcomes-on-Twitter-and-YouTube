...
output_folder = "/media/gabett/Volume/data-repository/panconesi-football-elections/football/youtube/trees/shuffled_graph_analysis_for_each_comment/"
...
video_ids_for_search = df_comments %>%
  inner_join(df_posts, by = c("video_id")) %>%
  group_by(video_id) %>%
  dplyr::summarise(n = n()) %>%
  ungroup()
