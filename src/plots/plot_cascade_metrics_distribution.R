...
plot_binnable_metrics <- df_metrics %>%
  ...
  labs(y = "Count",
       x = "",
       title = paste("Overall Distribution of cascade metrics on", str_to_title(social)),
       fill = "Topic") +
  theme_classic() +
  theme(
...
pdf_filename <- ifelse(
  shuffled_label == "",
  paste("plot", social, "overall_graph_metrics.pdf", sep = "_"),
  paste("plot", social, shuffled_label, "overall_graph_metrics.pdf", sep = "_")
)
