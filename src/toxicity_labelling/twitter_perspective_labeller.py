...
if __name__ == "__main__":
    ...
    df = pd.read_parquet("/media/gabett/Volume/data-repository/panconesi-football-elections/elections_missing_new_tweets_to_score_final_after_internet_outrage.parquet") # df = pd.read_csv(sys.argv[2], dtype=dtype)
    ...
    output_file = f"/media/gabett/Volume/data-repository/panconesi-football-elections/elections_missing_new_tweets_to_score_final_after_internet_outrage_scored.parquet" # output_file = sys.argv[4] + block_end + '.csv'
    df.to_parquet(output_file)